  ...
  parse(data: ArrayBuffer) {
    ...
    function parseHeader(data: string) {
      ...
      if (PCDheader.count !== null) {
        PCDheader.count = PCDheader.count[1].split(' ').map(function (
          x: string,
        ) {
          return parseInt(x, 10);
        });
    ...
    if (PCDheader.data === 'ascii') {
      ...
      for (let i = 0, l = lines.length; i < l; i++) {
        ...
        if (offset.rgb !== undefined) {
          const rgb_field_index = PCDheader.fields.findIndex(
            (field: string) => field === 'rgb',
          );
    ...
    if (PCDheader.data === 'binary') {
      ...
      for (
        ...
        if (offset.intensity !== undefined) {
          intensity.push(
            dataview.getFloat32(row + offset.intensity, this.littleEndian),
          );
        }
